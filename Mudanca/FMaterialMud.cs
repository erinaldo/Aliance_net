...
namespace Mudanca
{
    public partial class TFMaterialMud : Form
    {
        private CamadaDados.Mudanca.TRegistro_MaterialMud rmaterialmud;
        public CamadaDados.Mudanca.TRegistro_MaterialMud rMaterialMud
        {
            get
            {
                if (bsMaterialMud.Current != null)
                    return bsMaterialMud.Current as CamadaDados.Mudanca.TRegistro_MaterialMud;
                else
                    return null;
            }
            set { rmaterialmud = value; }
        }
        public string Cd_empresa
        { get; set; }

        public TFMaterialMud()
        {
            InitializeComponent();
        }
        ...
        private void TFMaterialMud_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        {
            string vParam = "|exists|(select 1 from tb_amx_almox_x_empresa x " +
                            "           where x.id_almox =  '" + id_almox.Text + "'" +
                            "           and x.cd_empresa = '" + Cd_empresa.Trim() + "')";
